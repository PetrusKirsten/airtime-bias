...
class LowerThirdVisualConfig:
    ...
    Generic rectangle/text cues produced false positives on sponsor boards and set
    elements. The strict detector therefore requires a horizontally continuous
    copper/rose band plus a concentrated two-line light-text pattern. Circular-logo
    and edge cues remain weak supporting evidence and cannot trigger a detection alone.
    """
...
def _copper_mask(hsv: np.ndarray, config: LowerThirdVisualConfig) -> np.ndarray:
    lower_1 = np.array(
        [config.copper_hue_low_1, config.copper_saturation_min, config.copper_value_min],
        dtype=np.uint8,
    )
    upper_1 = np.array(
        [config.copper_hue_high_1, config.copper_saturation_max, config.copper_value_max],
        dtype=np.uint8,
    )
    lower_2 = np.array(
        [config.copper_hue_low_2, config.copper_saturation_min, config.copper_value_min],
        dtype=np.uint8,
    )
    upper_2 = np.array(
        [config.copper_hue_high_2, config.copper_saturation_max, config.copper_value_max],
        dtype=np.uint8,
    )
...
def _expected_label_band(array: np.ndarray) -> np.ndarray:
    ...
    return array[
        int(0.28 * height) : int(0.92 * height),
        int(0.03 * width) : int(0.82 * width),
    ]
...
def _visual_gate(copper: dict, text: dict, config: LowerThirdVisualConfig) -> tuple[bool, str | None]:
    reasons: list[str] = []
...
def analyze_lower_third_frame(
    ...
    result = {
        ...
        **{
        ...
        "rectangle_x": int(round(0.03 * analysis_crop.shape[1])),
        "rectangle_y": int(round(0.28 * analysis_crop.shape[0])),
        "rectangle_width": int(round(0.79 * analysis_crop.shape[1])),
        "rectangle_height": int(round(0.64 * analysis_crop.shape[0])),
    }
    return result
